...
import { onboardingProjectBodySchema } from "@/lib/api-body-schemas";
...
export async function POST(req: Request) {
  try {
    ...
    if (mErr || !mem?.organization_id) {
      return NextResponse.json(
        { error: "יש להשלים קודם את שלב הארגון" },
        { status: 400 },
      );
    }
    ...
    const bv = onboardingProjectBodySchema.safeParse(rawBody);
    if (!bv.success) {
      return NextResponse.json({ error: bv.error.flatten() }, { status: 400 });
    }

    const name = sanitizeString(bv.data.name);
    const project_code = sanitizeString(bv.data.project_code).toUpperCase();
    if (!name || !project_code) {
      return NextResponse.json(
        { error: "שם פרויקט וקוד נדרשים" },
        { status: 400 },
      );
    }
    if (!/^[A-Z0-9_]{2,20}$/.test(project_code)) {
      return NextResponse.json(
        { error: "קוד פרויקט לא תקין" },
        { status: 400 },
      );
    }
    ...
    if (orgErr || !orgRow?.client_id) {
      return NextResponse.json(
        { error: "ארגון ללא לקוח — פנו לתמיכה" },
        { status: 400 },
      );
    }
    ...
    const { data: client, error: cErr } = await getClientPlanRow(
      admin,
      clientId,
    );
    if (cErr || !client) {
    ...
    console.error("[onboarding/project]", e);
